...
The shared-asset system is **prebuilt and ready to copy** in [lesson-kit/](./lesson-kit/) — `base.css`, `nav.js`, `quiz.js`, `shiki.js`, templates, a living `styleguide.html`, and its `DESIGN.md` / `DESIGN_SYSTEM.md`. Copy it into a workspace per [lesson-kit/README.md](./lesson-kit/README.md); do not hand-write the CSS, nav, or syntax-highlighting layer. Code blocks use Shiki; new workspaces should add `shiki` as an npm dependency and include the kit's `assets/shiki.js` module on lesson/reference pages. [DESIGN_REFERENCE.md](./DESIGN_REFERENCE.md) is the conceptual guide (when to use which mode, visual principles, anti-patterns); `effect-deep-dive` is the reference instance. If a workspace locks its own `DESIGN.md` / template, that binds and supersedes.
